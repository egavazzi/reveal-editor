...
import { convertAssetToWebm } from '../../src/client/lib/api.js'
...
describe('convertAssetToWebm', () => {
  it('reports progress and resolves to the final path', async () => {
    ...
    const path = await convertAssetToWebm('assets/clip.mov', { fetchImpl, onProgress: (p) => seen.push(p) })
  ...
  it('rejects with the server error from the stream', async () => {
    ...
    await expect(convertAssetToWebm('assets/clip.mov', { fetchImpl })).rejects.toThrow('ffmpeg exited with code 1')
  })

  it('rejects on a non-2xx response', async () => {
    const fetchImpl = async () => new Response(JSON.stringify({ error: 'ffmpeg is not installed on this machine' }), { status: 501 })
    await expect(convertAssetToWebm('assets/clip.mov', { fetchImpl })).rejects.toThrow('ffmpeg is not installed')
  })

  it('rejects when the stream ends without a result', async () => {
    const fetchImpl = async () => streamed(['{"progress":0.1}\n'])
    await expect(convertAssetToWebm('assets/clip.mov', { fetchImpl })).rejects.toThrow('without a result')
  })
})
